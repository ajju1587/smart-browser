...
import '../../ai/presentation/summaries_view.dart';
              ...
              return Padding(
                ...
                child: InputChip(
                  ...
                  onSelected: (_) async {
                    ...
                    // ensure the webview loads the selected tab URL
                    final urlToLoad = t.url;
                    if (_controller != null) {
                      try {
                        //await _controller!.loadUrl(urlRequest: URLRequest(url: Uri.parse(urlToLoad)));
                        await _controller!.loadUrl(urlRequest: URLRequest(url: WebUri.uri(Uri.parse(urlToLoad))));
                  ...
                  onDeleted: () async {
                    ...
                    final newActive = ref.read(tabManagerProvider).activeTab;
                    if (newActive != null && _controller != null) {
                      try {
                        await _controller!.loadUrl(urlRequest: URLRequest(url: WebUri.uri(Uri.parse(newActive.url))));
                ...
                Padding(
                  ...
                  child: ActionChip(
                    ...
                    onPressed: () async {
                      ...
                      // load the new tab URL in the existing webview
                      if (_controller != null) {
                        try {
                          //await _controller!.loadUrl(urlRequest: URLRequest(url: Uri.parse(newUrl)));
                          await _controller!.loadUrl(urlRequest: URLRequest(url: WebUri.uri(Uri.parse(newUrl))));
                        } catch (_) {}
                      }
                    },
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                onPressed: () async {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Download queued: ${meta.name ?? meta.sourceUrl}')));
                },
              ),
              ElevatedButton.icon(
              ...
              ElevatedButton.icon(
                ...
                onPressed: () {
                  Navigator.of(context).push(MaterialPageRoute(builder: (_) => const SummariesView()));
                },
              ),
              ElevatedButton.icon(
